        ...
        local function initializeFreecam()
            ...
            local function logicThread()
                while isFreecamActive do
                    ...
                    if IsDisabledControlJustPressed(0, 24) then -- عند الضغط على زر التفاعل (غالباً كليك يسار)
    local currentFeature = Features[currentFeatureIndex]
    
    -- أبقينا فقط ميزة الانتقال
    if currentFeature == "Teleport" and targetCoords then
        local ped = PlayerPedId()
        
        -- الحصول على ارتفاع الأرض بدقة لضمان عدم السقوط تحت الخريطة
        local _, z = GetGroundZFor_3dCoord(targetCoords.x, targetCoords.y, targetCoords.z + 1.0, false)
        
        -- تنفيذ الانتقال إلى إحداثيات الكاميرا
        SetEntityCoords(ped, targetCoords.x, targetCoords.y, z and z + 1.0 or targetCoords.z, false, false, false, true)
        
        -- اختياري: إرسال إشعار بسيط للتأكيد
        -- MachoMenuNotification("[INFO]", "تم الانتقال بنجاح")
    end
endend

            -- Main Camera Movement Thread (Unchanged)
            local function cameraThread()
                local baseSpeed, boostSpeed, slowSpeed = 0.5, 3.0, 0.1; local mouseSensitivity = 7.5; local function clamp(val, min, max) return math.max(min, math.min(max, val)) end; local function rotToDir(rot) local rX, rZ = math.rad(rot.x), math.rad(rot.z); return vector3(-math.sin(rZ)*math.cos(rX), math.cos(rZ)*math.cos(rX), math.sin(rX)) end;
                while isFreecamActive do
                    Wait(0)
                    ...
                    SetCamCoord(freecamHandle, camPos.x, camPos.y, camPos.z); SetCamRot(freecamHandle, camRot.x, camRot.y, camRot.z, 2); 
